...
namespace Ucla.Common.ExtensionMethods
{
    public static class AdoExtensions
    {
        ...
        /// <summary>
        /// Convert a database value to a Byte[]
        /// </summary>
        /// <remarks>
        /// In MS SQL Server the corresponding data type is varbinary(max).
        /// Since an Array is a reference type, a database null value
        /// should always map to a null reference in C#.
        /// </remarks>
        /// <param name="dr">DataReader</param>
        /// <param name="colName">Column Name</param>
        /// <returns>The converted value</returns>
        public static Byte[] AsByteArray(this IDataReader dr, string colName)
        {
            try
            {
                int ordinal = dr.GetOrdinal(colName);
                return dr.IsDBNull(ordinal) ? null : (byte[])dr[ordinal];
            }
            catch (IndexOutOfRangeException exo)
            {
                var message = String.Format(NoColumnMessage, colName);
