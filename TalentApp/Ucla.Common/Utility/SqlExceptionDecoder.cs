    ...
    public static class SqlExceptionDecoder
    {
        public static string GetFriendlyMessage(string objectName, Exception ex)
        {
            ...
            if (ex.Message.Contains("DELETE") && ex.Message.Contains("REFERENCE"))
                msg.AppendFormat(
                    "You cannot delete this {0} as it is referenced by other items in the application.", 
                    objectName);
            else if (ex.Message.Contains("String or binary data would be truncated"))
                msg.AppendFormat(
                    "One or more text inputs is too long for the database to save.");
            else
                msg.AppendFormat(
                    "Save {0} failed. The database returned the error message {1}",
